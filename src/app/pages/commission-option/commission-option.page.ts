  ...
  async getCountries() {
   // let result: any = await this.apiSer.getCountryList();
    ...
    if (this.cardPaymentoption.iban != '') {
      ...
      let selectedCountry = this.countries.filter(item => item.IBAN == this.cardPaymentoption.iban.slice(0, 2))[0].countryCode;
      console.log(selectedCountry);
     if(selectedCountry != undefined){
      this.maxLength = selectedCountry.totalLength;
      this.form.get("ibancode").setValue(selectedCountry);
